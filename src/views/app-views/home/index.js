...
const Home = (props) => {
	...
	const [showNewMarker, setShowNewMarker] = useState(true);
	const [showMarker1, setShowMarker1] = useState(true);
	const [showMarker2, setShowMarker2] = useState(true);
	const [showSOSMarker, setShowSOSMarker] = useState(true);
	...
	const deleteRequest = () => {
		if (currentMarkerData.createdBy === user.token) {
			//kiểm tra có đúng là người tạo không
			db.collection('sos-requests')
				.doc(currentMarkerData.id)
				.update({
	...
	const getAllRequest = () => {
		if (windyMap && markerGroup) {
			//Tạo icon SOS
			var CustomSOSIcon = window.L.Icon.extend({
				options: {
					iconSize: [30, 50],
				},
			});
			...
			var sosIcon_green = new CustomSOSIcon({
				iconUrl: '/img/SOS_Green.png',
			});
			var sosIcon_yellow = new CustomSOSIcon({
				iconUrl: '/img/SOS_Yellow.png',
			});
			var sosIcon_orange = new CustomSOSIcon({
				iconUrl: '/img/SOS_Orange.png',
			});
			...
			db.collection('sos-requests')
				...
				.onSnapshot((querySnapshot) => {
					...
					querySnapshot.forEach((doc) => {
						if (
							!viewList.length ||
							(viewList.length && viewList.indexOf(doc.id) !== -1)
						) {
							var data = doc.data();
							var icon = null;
							...
							if (
								moment(data.createdAt?.toDate()).add(
									10,
									'day'
								) < moment()
							) {
	...
	const getCurrentLocation = () => {
		...
		//tạo icon
		var CustomIcon = window.L.Icon.extend({
			options: {
				iconSize: [20, 20],
			},
		});
		...
		window.L.marker([props.coords.latitude, props.coords.longitude], {
			icon: locationIcon,
		}).addTo(currentPositionGroup);
	...
	const onTabsChange = (key) => {
		if (key.includes('3')) {
			//nếu người dùng nhấn vào tab thứ 3 - tab Thời tiết
			if (!weatherData.ts) {
				fetch('https://api.windy.com/api/point-forecast/v2', {
					//gọi tới api của Windy
					method: 'POST',
					headers: {
					...
					body: JSON.stringify({
						...
						parameters: [
							...
							'windGust',
							'waves',
						], //thông tin thời tiết muốn lấy về
						levels: ['surface'],
	...
	const getPlanList = () => {
		...
		return db
			.collection('plans')
			.where('createdByToken', '==', user.token)
			.get()
			.then((querySnapshot) => {
				if (querySnapshot.size) {
					setPlanData(
						querySnapshot.docs.map((plan) => {
							return {
								...plan.data(),
								id: plan.id,
							};
			...
			.then((parent) => {
				parent.map((plan) => {
					if (
						'locations' in plan.data() &&
						plan.data().locations.length
					) {
	...
	const removeRequestFromPlan = () => {
		if (selectedPlan.isLeaf) {
			...
			db.collection('plans')
				...
				.update({
					locations:
						firebase.firestore.FieldValue.arrayRemove(childKey),
				})
				.then(() => {
					onShowPlanModal();
	...
	const removePlan = () => {
		...
		if (!selectedPlan.isLeaf) {
			//kiểm tra xem đang chọn kế hoạch hay điểm đến
			db.collection('plans')
				.doc(selectedPlan.key)
				.delete()
				.then(() => {
		...
		checkedKeys.forEach((plan) => {
			...
			if (plan.locations?.length) {
				plan.locations.forEach((location) => {
					if (requestList.indexOf(location) === -1)
						requestList.push(location);
				});
			}
		});
	...
	const onRouting = () => {
		if (props.coords) {
			var pointList = Object.keys(markerGroup._layers)
				.map(
					(pointId) =>
						+markerGroup._layers[pointId]._latlng.lat +
						',' +
						markerGroup._layers[pointId]._latlng.lng
				)
				.join('%7C');
			window.open(
	...
	useEffect(() => {
		...
		if (!initMap) {
			...
			window.windyInit(options, (windyAPI) => {
				...
				var topLayer = window.L.tileLayer(
				...
				if (typhoons.length) {
					typhoons.forEach((typhoon) => {
						typhoon.predict.forEach((dateData) => {
							var eyeStorm = window.L.marker(
								[dateData.position.lat, dateData.position.lon],
								{ icon: eyeIcon }
							).addTo(typhoonGroup); //tạo mắt bão

							eyeStorm.bindPopup(
								'<p><b>Tên bão: ' +
									typhoon.name +
									'</b><br>Cập nhật gần nhất: ' +
									moment(typhoon.lastUpdate).format(
										'DD-MM-YYYY HH:mm'
									) +
									'</p>' +
									'<br><b>Dự đoán cho ngày ' +
									moment(dateData.timestamp).format(
										'DD-MM-YYYY'
									) +
									' </b></p>' +
									'<br>Áp suất không khí tại tâm bão: ' +
									dateData.hPa +
									' hPa</p>' +
									'<br>Tốc độ gió tối đa: ' +
									dateData.maxSusWind[0] +
									' m/s ~ ' +
									dateData.maxSusWind[1] +
									' km/s</p>' +
									'<br>Hướng di chuyển: ' +
									dateData.movingDirection +
									'</p>' +
									'<br>Tốc độ di chuyển: ' +
									dateData.movingSpeed +
									' km/h</p>' +
									'<br>Bán kín vùng có sức gió trên 15m/s: ' +
									dateData.radius15 +
									' km</p>' +
									'<br>Bán kính vùng có sức gió trên 25m/s: ' +
									dateData.radius25 +
									' km</p>'
							); //tạo thông tin khi nhấn vào mắt bão

							var circle15 = window.L.circle(
								[dateData.position.lat, dateData.position.lon],
								{
									color: 'blue',
									weigth: '1',
							...
							if (dateData.radius25) {
								var circle25 = window.L.circle(
									[
										dateData.position.lat,
										dateData.position.lon,
									],
									{
										color: 'red',
										weigth: '1',
				...
				var searchControl =
					window.L.esri.Geocoding.geosearch().addTo(map);
				...
				searchControl.on('results', function (data) {
					...
					for (var i = data.results.length - 1; i >= 0; i--) {
						results.addLayer(
							window.L.marker(data.results[i].latlng)
						);
					}
				});
	...
	return (
		<>
			...
			>
				<div>
					<Checkbox
						checked={showNewMarker}
						onChange={(e) => {
							setShowNewMarker(e.target.checked);
						}}
					>
						Hiển thị địa điểm mới
					</Checkbox>
				</div>
				<div style={{ marginTop: 10 }}>
					<Checkbox
						checked={showSOSMarker}
						onChange={(e) => {
							setShowSOSMarker(e.target.checked);
						}}
					>
						Hiển thị địa điểm đã qua 10 ngày chưa được hỗ trợ
					</Checkbox>
				</div>
				<div style={{ marginTop: 10 }}>
					<Checkbox
						checked={showMarker1}
						onChange={(e) => {
							setShowMarker1(e.target.checked);
						}}
					>
						Hiển thị địa điểm đã nhận hỗ trợ 1 lần
					</Checkbox>
				</div>
				<div style={{ marginTop: 10 }}>
					<Checkbox
						checked={showMarker2}
						onChange={(e) => {
							setShowMarker2(e.target.checked);
						}}
					>
						Hiển thị địa điểm đã nhận hỗ trợ 2 lần trở lên
					</Checkbox>
				</div>
			</div>
						...
						rules={[
							{
								required: true,
								message:
									'Vui lòng cho biết tên người / hộ gia đình cần giúp đỡ!',
							},
						]}
					>
						<Input />
						...
						rules={[
							{
								required: true,
								message:
									'Vui lòng cho biết số điện thoại cần giúp đỡ!',
							},
						]}
					>
						<Input />
						...
						rules={[
							{
								required: true,
								message:
									'Vui lòng cho biết địa chỉ cần giúp đỡ!',
							},
						]}
					>
						<Input />
				...
				>
					<Collapse.Panel header="Thông tin chung" key="1">
						<p>
							Ngày đăng tin:{' '}
							{moment(
								currentMarkerData.createdAt?.toDate()
							).format('DD-MM-YYYY HH:mm')}
						</p>
						<p>
							Người / Hộ gia đình cần giúp đỡ:{' '}
							{currentMarkerData.username}
						</p>
						<p>Số điện thoại: {currentMarkerData.phone}</p>
						<p>Địa chỉ: {currentMarkerData.address}</p>
						<p>
					...
					<Collapse.Panel header="Thời tiết" key="3">
						<Row className="mb-3">
							<Pagination
								className="m-auto"
								simple
								defaultCurrent={currentWeatherPage}
								total={200}
								onChange={(page) => setCurrentWeatherPage(page)}
							/>
						</Row>
						<Row gutter={16}>
							{/* Hiển thị thời tiết  */}
							{weatherData.ts
								?.slice(
									(currentWeatherPage - 1) * 4,
									(currentWeatherPage - 1) * 4 + 4
								)
								.map((timestamp, index) => (
									<Col span={6}>
										<Card
										...
										>
											...
											<p>
												Vận tốc gió lúc giật:{' '}
												{weatherData['gust-surface'][
													(currentWeatherPage - 1) *
														4 +
														index
												].toFixed(2)}{' '}
					...
					<Collapse.Panel header="Danh sách hỗ trợ" key="4">
						<Table
							className="mb-3"
							dataSource={currentMarkerData.donations}
							columns={columns}
						/>
					</Collapse.Panel>
				</Collapse>
				{currentMarkerData.createdBy === user.token ? (
					<Button className="mr-3" danger onClick={deleteRequest}>
						Xóa yêu cầu
					</Button>
				) : null}
				<Button
					className="mr-3"
					danger
					onClick={() => reportRequest(currentMarkerData)}
				>
					Báo cáo giả mạo
				</Button>
				<Button
					...
					onClick={() =>
						window.open(
							'https://www.google.com/maps/dir/?api=1&origin=' +
								...
								props.coords.longitude +
								'&destination=' +
								currentMarkerData.lat +
								',' +
								currentMarkerData.lng +
								'&travelmode=driving'
						)
					}
				>
			...
			<Modal
				...
				okButtonProps={{ style: { display: 'none' } }}
			>
				<Form layout="vertical" onFinish={onDonationSubmit}>
					<Form.Item label="Hỗ trợ tiền:" name="money">
						<Input />
					</Form.Item>
					<Form.Item label="Hỗ trợ hiện vật" name="items">
						<Input.TextArea rows={4} />
					</Form.Item>
					<Form.Item>
						<Button type="primary" htmlType="submit">
			...
			<Modal
				...
				footer={
					<>
						<Button onClick={removePlan} danger>
							Xóa kế hoạch
						</Button>
						<Button onClick={removeRequestFromPlan} danger>
							Xóa điểm đến
						</Button>
						<Button
						...
						<Button onClick={onAddToPlan} type="primary">
							Thêm điểm đến
						</Button>
					</>
				}
			...
			<Modal
				...
				footer={
					<>
						...
						<Button onClick={showSummary} type="primary">
							Xem tổng kết
						</Button>
						<Button
							onClick={() => setShowAddPlan(true)}
				...
				>
					<Form.Item
						...
						rules={[
							{
								required: true,
								message: 'Vui lòng cho biết tên kế hoạch!',
							},
						]}
					>
						<Input />
